...
def make_structure_dict(atoms_dataframe):
    ...
    for row in tqdm.tqdm(atoms.itertuples()):
...
def enhance_structure_dict(structure_dict):
    ...
    for molecule_name in structure_dict:
        ...
        molecule['sublabel_atom'] = ['-'.join([molecule['long_symbols'][atom_index]]
        ...
        molecule['heavyvalences'] = [mol.atoms[i].heavydegree for i in range(n_atom)]
        #molecule['heterovalences'] = [mol.atoms[i].heterovalence for i in range(n_atom)]
        molecule['heterovalences'] = [mol.atoms[i].heterodegree for i in range(n_atom)]
        #molecule['valences'] = [mol.atoms[i].valence for i in range(n_atom)]
        molecule['valences'] = [mol.atoms[i].degree for i in range(n_atom)]
...
def enhance_atoms(atoms_dataframe, structure_dict):
    ...
    keys = ['distances','angle', 'bond_orders', 'top_bonds', 'bond_ids', 'long_symbols','sublabel_atom',
                'charges', 'spins', 'heavyvalences', 'heterovalences', 'valences', 'hyb_types']
    newkeys = [k if k[-1] != 's' else k[:-1] for k in keys]

    def __func(x):
        return [structure_dict[x['molecule_name']][key][x['atom_index']] for key in keys]
...
def write_csv(directory, label='', atoms=None, bonds=None, triplets=None,
              quadruplets=None):
    ...
    if atoms is not None and len(atoms) > 0:
        ...
        atoms.to_csv(filename.format('structures'), index=False,
                     columns='molecule_name,atom_index,atom,x,y,z,labeled_atom,angle,top_bond_0,top_bond_1,top_bond_2,top_bond_3,sublabel_atom,charge,spin,heavyvalence,heterovalence,valence,hyb_type'.split(','))
    ...
    if quadruplets is not None and len(quadruplets) > 0:
...
def add_bonds_embedding_type(bonds, triplets, quadruplets):
    # Add the embedding info to the dataframes.
...
def add_atoms_embedding(atoms, embeddings):
...
def get_scaling(bonds_train):
    ...
    res = bonds_train.groupby("labeled_type")["scalar_coupling_constant"].aggregate(['mean', 'std']).to_dict()
    #stds = bonds_train.groupby("labeled_type")["scalar_coupling_constant"].std().to_dict()
...
def create_dataset(atoms, bonds, triplets, quads, labeled=True, max_count=10**10):
    ...
    Returns:
        tuple: With the following entries.
            ...
            * x_triplet: (M,P,7): Triplet type (2), Atom index (3), Bond index (2) corresponding to the triplet.
            * x_triplet_angle: (M,P) Triplet angle.
            * x_quad: (M,Q,10) Quadruplet type (1), Atom index (4), Bond index (3), and triplet index (2) corresponding to the quadruplet.
            * x_quad_angle: (M,Q) Quadruplet dihedral angle.
            * y_bond_scalar_coupling: (M,B,4) of the scalar coupling constant, type mean, type std, and whether it should be predicted.
...
def auto_preproc_stage1():
    ...
    def process_bonds(label):
        ...
        print('Writing bonds...')
        write_csv(root/settings['PROCESSED_DATA_DIR'], label=label, bonds=bonds,
...
def auto_preproc_stage2():
    import torch
    print("Loading data...")
    os.chdir(root/settings['PROCESSED_DATA_DIR'])

    atoms = pd.read_csv('new_big_structures.csv.bz2')

    print('Loading train data...')
    bonds_train = pd.read_csv('new_big_train.csv.bz2')
    triplets_train = pd.read_csv('new_big_train_triplets.csv.bz2')
    quadruplets_train = pd.read_csv('new_big_train_quadruplets.csv.bz2')

    print('Loading test data...')
    bonds_test = pd.read_csv('new_big_test.csv.bz2')
    triplets_test = pd.read_csv('new_big_test_triplets.csv.bz2')
    quadruplets_test = pd.read_csv('new_big_test_quadruplets.csv.bz2')

    print('Sorting...')
    atoms.sort_values(['molecule_name','atom_index'], inplace=True)
    bonds_train.sort_values(['molecule_name','atom_index_0','atom_index_1'], inplace=True)
    triplets_train.sort_values(['molecule_name','atom_index_0','atom_index_1','atom_index_2'], inplace=True)
    quadruplets_train.sort_values(['molecule_name','atom_index_0','atom_index_1','atom_index_2','atom_index_3'], inplace=True)

    bonds_test.sort_values(['molecule_name','atom_index_0','atom_index_1'],inplace=True)
    triplets_test.sort_values(['molecule_name','atom_index_0','atom_index_1','atom_index_2'],inplace=True)
    quadruplets_test.sort_values(['molecule_name','atom_index_0','atom_index_1','atom_index_2','atom_index_3'],inplace=True)

    # add embedding type columns
    add_atoms_embedding_type(atoms)
    add_bonds_embedding_type(bonds_train, triplets_train, quadruplets_train)
    add_bonds_embedding_type(bonds_test, triplets_test, quadruplets_test)

    bonds = pd.concat([bonds_train, bonds_test], sort=False)
    triplets = pd.concat([triplets_train, triplets_test], sort=False)
    quadruplets = pd.concat([quadruplets_train, quadruplets_test], sort=False)
    ...
    # create embedding
    embeddings = create_embedding(atoms, bonds, triplets, quadruplets)

    print("Adding embeddings and scaling...")
    add_atoms_embedding(atoms, embeddings)

    means, stds = get_scaling(bonds_train)
    def __add_embeddings_and_scaling(bonds, triplets, quadruplets):
        add_bonds_embedding(bonds, triplets, quadruplets, embeddings)
        add_scaling(bonds, means, stds)

    __add_embeddings_and_scaling(bonds_train, triplets_train, quadruplets_train)
    __add_embeddings_and_scaling(bonds_test, triplets_test, quadruplets_test)

    print("Creating train dataset...")
    D = create_dataset(atoms, bonds_train, triplets_train, quadruplets_train, labeled=True)
    ...
    D_sub = create_dataset(atoms, bonds_test, triplets_test,
                           quadruplets_test, labeled=False)
